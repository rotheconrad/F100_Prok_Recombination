...
def parse_blastani(blastin, min_ani, min_frac):

    # Reads blastani tsv file, filters results, returns dict

    # {read-pair: [ani, shared_frac]}
    blast_dict = {}

    with open(blastin, 'r') as file:
        ...
        for line in file:
            ...
            if qry == ref: continue
            ani = float(X[4])
            shared_frags = int(X[7])
            total_frags = min(int(X[8]), int(X[9]))
            shared_frac = shared_frags / total_frags
            ...
            if shared_frac >= min_frac and ani >= min_ani:
                ...
                if sorted_pair in blast_dict:
                    old_ani = blast_dict[sorted_pair][0]
                    if ani > old_ani:
                        blast_dict[sorted_pair] = [ani, shared_frac]
                else:
                    blast_dict[sorted_pair] = [ani, shared_frac]
...
def main():
    ...
    blastin = args['input_blastani_file']
    ...
    if not title:
        f = fastin.split('/')[-1].split('.')[0]
        b = blastin.split('/')[-1].split('.')[0]
        title = f"{f} - {b}"

    # parse the fastANI file - returns dict {read_pair: [ANI, shared_fraction]}
    print('\nParsing fastANI file ...\n')
    fastANI = parse_fastani(fastin, min_ani, min_frac)

    # parse the blastANI file - returns dict {read_pair: [ANI, shared_fraction]}
    print('\nParsing Blast ANI file ...\n')
    blastANI = parse_blastani(blastin, min_ani, min_frac)

    # compare the two
    df = compute_difference(fastANI, blastANI)
