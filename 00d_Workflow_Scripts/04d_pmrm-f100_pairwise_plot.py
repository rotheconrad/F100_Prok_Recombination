...
This script plots pm/rm vs F100 from the pairwise all vs. all pmrm data.

It builds a figure similar to 02d_f100_scatter_pyGAM-fit.py but with the
difference that pmrm is plotted on the y-axis against F100.
...
def pmrm_f100_plot(df, xmin, xmax, xt, ymin, ymax, yt, z, p, a, gam, mm, lg, op):

    # Gather Stats
    df_stats = gather_stats(df['f100'], df['pmrm'])
    ...
    # build plot
    g = sns.JointGrid(x="f100", y="pmrm", data=df)
    ...
    ylab = 'ln(ρ/θ)' if lg else 'ρ/θ'
    g.ax_joint.set_ylabel(
        ylab,
        fontsize=12, x=-0.02
        )
    g.ax_joint.text(
        0.27, 0.99, stats_line,
        fontsize=10, color=second_color,
        verticalalignment='top', horizontalalignment='right',
        transform=g.ax_joint.transAxes
        )
...
def main():
    ...
    parser.add_argument(
        '-xmin', '--xaxis_minimum',
        help='OPTIONAL: Minimum value to plot on x-axis. (Default=0.0)',
        metavar='.',
        type=float,
        default=0.0,
        required=False
    ...
    parser.add_argument(
        '-xmax', '--xaxis_maximum',
        help='OPTIONAL: Maximum value to plot on x-axis. (Default=1.0)',
        metavar='.',
        type=float,
        default=1.0,
        required=False
    ...
    parser.add_argument(
        '-xt', '--xaxis_step_size',
        help='OPTIONAL: x-axis ticks step increment. (Default=0.1)',
        metavar='.',
        type=float,
        default=0.1,
        required=False
    ...
    # build plots
    _ = pmrm_f100_plot(df, xmin, xmax, xt, ymin, ymax, yt, z, p, a, gam, mm, lg, op)
