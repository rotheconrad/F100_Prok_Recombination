...
def parse_ANI_file(infile):
    ...
    anilist = [] # track ani values to print min and max.

    # read the file, compute distance 100 - ANI, store data in dist dict
    with open(infile, 'r') as file:
        #header = file.readline() # remove header
        for line in file:
            X = line.rstrip().split('\t')
            g1 = X[0].split('.')[0]
            g2 = X[1].split('.')[0]
            ...
            if gpair in dist:
                old_sfrac = dist[gpair][1]
                if sfrac >= old_sfrac:
                    dist[gpair] = [d, sfrac]
            else:
                dist[gpair] = [d, sfrac]
            # add all genome names to gdict to get the complete set of names
            # us this to check we have complete all vs all ANI estimates
            gdict[g1] = ''
            gdict[g2] = ''
...
def plot_clustred_heatmap(df, outfile, metadf, cdict):
    ...
    ticks = [95.75,96.25,96.75,97.25,97.75,98.25,98.75,99.25,99.75]
